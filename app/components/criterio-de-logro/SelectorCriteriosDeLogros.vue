...
>
  <template #content>
    ...
    <UCard class="flex flex-col flex-1 overflow-hidden min-w-0">
      ...
      <!-- BODY SCROLLABLE -->
      <div class="flex-1 overflow-y-auto min-h-0 max-h-[60vh] overflow-y-auto">
        ...
        >
          <li
            v-for="(criterioDeLogro) in criteriosDeLogrosFiltered"
            :key="criterioDeLogro.id"
            @click="onToggleCriterioDeLogro(criterioDeLogro)"
          ...
          >
            ...
            <div class="flex items-center gap-3">
              <UPopover
                ...
                mode="hover"
                v-if="criterioDeLogro.recomendado"
              >
                <template #default="{ open }">
                  <UTooltip :prevent="open">
                    <UButton
                      label="Recomendado"
                  ...
                  <div class="p-4 flex flex-col gap-y-4 max-w-128">
                    ...
                    <div v-if="criterioDeLogro.contenidoRelacionado">
                      ...
                      <ul class="list-disc ml-4">
                        <li class="my-2">
                          {{ criterioDeLogro.contenidoRelacionado?.descripcion }}
                        </li>
                      </ul>
                    </div>
                    ...
                    <div v-if="criterioDeLogro.competenciasEspecificasRelacionadas?.length > 0">
                      <span class="font-medium">
                        Se relaciona a las competencias especificas seleccionadas:
                      </span>
                      <ul class="list-disc ml-4">
                        <li
                          v-for="(competenciaEspecifica,idx) in criterioDeLogro.competenciasEspecificasRelacionadas"
                          :key="idx"
                          class="my-2"
                        >
