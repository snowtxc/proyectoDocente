...
const onChangeContenido = (contenido: Contenido) => {
    // Solo asigna si es diferente al actual
    if (contenidoIdSelected.value !== contenido.id) {
        contenidoIdSelected.value = contenido.id;
    } else {
        // Si se hace clic en el mismo, se deselecciona (opcional)
        contenidoIdSelected.value = null;
    }
}
...
const handleCheckboxChange = (contenidoId: number) => {
    if (contenidoIdSelected.value === contenidoId) {
        // Si ya está seleccionado, lo deseleccionamos
        contenidoIdSelected.value = null;
    } else {
        // Si no está seleccionado, lo seleccionamos (solo uno)
...
<template>
    <UButton icon="tabler:pencil" size="sm" color="primary" variant="outline" @click="isOpen = true"
            ...
            <UCard class="min-w-0">
                <template #header>
                    <div class="flex gap-2 items-center mt-2">
                        <UInput v-model="q" icon="i-heroicons-magnifying-glass" placeholder="Buscar Contenido" autofocus
                            class="flex-1" />
                        <UButton icon="tabler:x" size="sm" color="primary" square variant="solid" class="flex-none"
                            @click="isOpen = false;" />
                    </div>
                </template>
                        ...
                        <li v-for="(contenido) in contenidosFiltered" :key="contenido.id"
                            ...
                            <div class="flex items-center gap-3 w-full hover:cursor-pointer"
                                ...
                                <div class="text-sm min-w-0 flex gap-2 items-center">
                                    <!-- Checkbox modificado para comportamiento de radio -->
                                    <UCheckbox 
                            ...
                            <div class="flex items-center gap-3">
                                <UPopover :popper="{ placement: 'bottom-start' }" mode="hover" v-if="contenido.recomendado">
                                      <UTooltip>
                                          <UButton label="Recomendado" icon="tabler:butterfly-filled"
                                              :color="getColorBadgeComponente(props.color)" variant="outline" />
                                      </UTooltip>

                                      <template #content>
                                        <div class="p-2 m-4 flex flex-col gap-y-4 max-w-128">
                                            <div v-if="contenido.competenciasEspecificasRelacionadas?.length > 0">
                                                <span class="font-medium"> Se relaciona a las competencias especificas
                                                    seleccionadas: </span>
                                                <ul class="list-disc">
                                                    <li v-for="(competenciaEspecifica,idx) in contenido.competenciasEspecificasRelacionadas"
                                                        :key="idx" class="my-2"> {{ competenciaEspecifica.codificacion }}
                                                        {{ competenciaEspecifica.descripcion }}</li>
                                                </ul>
                                            </div>
                                            <USeparator color="primary" type="solid" />

                                            <div v-if="contenido.criteriosDeLogrosRelacionados?.length > 0">
                                                <span class="font-medium"> Se relaciona a los criterios de logros
                                                    seleccionados: </span>
                                                <ul class="list-disc">
                                                    <li v-for="(criterioDeLogro,idx) in contenido.criteriosDeLogrosRelacionados"
                                                        :key="idx" class="my-2">{{ criterioDeLogro.descripcion }}</li>
                                                </ul>
                                            </div>
                                        </div>
                                    </template>
                ...
                <template #footer>
                    <div class="flex justify-end gap-3 ">
                        ...
                        <UButton type="button" label="Guardar" color="primary"
                            :disabled="contenidoIdSelected == null" @click="onSelectContenido" />
                    </div>
                </template>
            </UCard>
        </template>
