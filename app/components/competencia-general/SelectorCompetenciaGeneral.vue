...
import type { Contenido } from '~/types/contenido';
  ...
  const getLista = () : CompetenciaGeneralItemSelector[] =>{
    ...
    return props.competenciasGenerales.map(competenciaGeneral =>{
        ...
        if(props.competenciasEspecificasSelected){
            ...
            //  competenciasEspecificasRealcionadas = competenciaEspecifica.criterios_de_logros.filter(criterioDeLogro => {
  ...
  const onSave = ()=>{
    const competenciasGeneralesSelected = competenciasGeneralesFiltered.value.filter(cdl => cdl.checked);
    emit('update:model-value', competenciasGeneralesSelected);
    isOpen.value = false;
  }

  const onToggle = (competenciaGeneral: CompetenciaGeneral & {
      checked: boolean;
  })=>{
      ...
      <UCard class="min-w-0 min-h-[75vh]">
        ...
        <div class="w-full overflow-y-auto max-h-full h-[75vh]">
          ...
          <ul v-else role="list" class="divide-y divide-gray-200 dark:divide-gray-800 overflow-y-auto">
            <li v-for="(competenciaGeneral) in competenciasGeneralesFiltered" :key="competenciaGeneral.id" @click="onToggle(competenciaGeneral)"
              ...
              <div class="flex justify-between items-center gap-3 w-full hover:cursor-pointer">
                ...
                <UAvatar :src="competenciaGeneral.url_image" size="md" class="text-center mt-2" />
                    ...
                    <template #content>
                        <div class="p-2 m-4 flex flex-col gap-y-4 max-w-64">
                            ...
                            <div v-if="competenciaGeneral.competenciasEspecificasRelacionadas?.length > 0">
                                <span class="font-medium"> Se relaciona a las siguientes competencias especificas:  </span>
                                <ul class="list-disc">
                                    <li v-for="(competenciaEspecifica,idx) in competenciaGeneral.competenciasEspecificasRelacionadas" :key="idx"> {{ competenciaEspecifica.descripcion }}</li>
                                </ul>
                            </div>
